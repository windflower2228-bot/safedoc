...
export async function POST(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  // 버전 이력
  await admin.from('document_versions').insert({ company_id:profile!.company_id, doc_type:'safety_document', doc_id:data.id, doc_number:docNumber, version:1, change_summary:'최초 작성', snapshot:data, author_id:user.id })
