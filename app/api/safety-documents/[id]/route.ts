...
export async function PATCH(req: NextRequest, { params }: P) {
  ...
  const { data, error } = await supabase.from('safety_documents').update({...upd,updated_at:new Date().toISOString()}).eq('id',params.id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  const { count } = await supabase.from('document_versions').select('*',{count:'exact',head:true}).eq('doc_id',params.id)
  await supabase.from('document_versions').insert({ company_id:profile!.company_id, doc_type:'safety_document', doc_id:params.id, doc_number:data.doc_number, version:(count??0)+1, change_summary:change_summary??'내용 수정', snapshot:data, author_id:user.id })
  return NextResponse.json({ data })
}
export async function DELETE(_: NextRequest, { params }: P) {
  ...
  await supabase.from('safety_documents').delete().eq('id',params.id)
