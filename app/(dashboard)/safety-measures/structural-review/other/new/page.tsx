...
export default function NewPage() {
  ...
  async function onSubmit(data: any) {
    ...
    const res = await fetch('/api/safety-measures/structural-review', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ ...data, review_type:'other', files:uploaded }) })
    const json = await res.json()
    setSaving(false)
    if (!res.ok) { toast.error(json.error); return }
    toast.success('구조검토서가 등록되었습니다.')
    router.push('/safety-measures/structural-review/other')
  }
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/safety-measures/structural-review/other" className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"><ArrowLeft className="w-4 h-4"/></Link>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Layers className="w-5 h-5" style={{color:'#6b7280'}}/>기타 구조검토서 등록</h1>
        </div>
        <button onClick={form.handleSubmit(onSubmit)} disabled={saving} className="btn-primary" style={{background:'#6b7280'}}>
