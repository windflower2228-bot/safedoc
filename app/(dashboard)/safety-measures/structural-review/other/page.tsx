...
export default function Page() {
  ...
  useEffect(()=>{fetch('/api/safety-measures/structural-review?type=other').then(r=>r.json()).then(j=>{setItems(j.data??[]);setLoading(false)})}, [])
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            <Layers className="w-5 h-5" style={{color:'#6b7280'}}/>기타 구조검토 및 조립상세도
          </h1>
        </div>
        <Link href="/safety-measures/structural-review/other/new" className="btn-primary text-sm gap-1.5" style={{background:'#6b7280'}}>
      ...
      <div className="card overflow-hidden">
        ...
        : items.length===0 ? (
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>등록된 구조검토서가 없습니다.</p>
            <Link href="/safety-measures/structural-review/other/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#6b7280'}}>
