...
export default function NewPage() {
  ...
  async function save() {
    ...
    setSaving(true)
    const res = await fetch('/api/health-programs/respiratory', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) })
    const json = await res.json()
    setSaving(false)
    if (!res.ok) { toast.error(json.error); return }
    toast.success('호흡기보호프로그램이 저장되었습니다.')
    router.push('/health-programs/respiratory')
  }
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-gray-900">호흡기보호프로그램 수립</h1>
          <p className="text-xs text-gray-400 mt-0.5">KOSHA GUIDE H-82-2020</p></div>
        </div>
      ...
      <div className="space-y-4">
        <div className="card p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="label-base">프로그램 제목 *</label><input value={data.title} onChange={e=>setData((d: any)=>({...d,title:e.target.value}))} placeholder="호흡기보호프로그램 제목" className="input-base"/></div>
