    ...
    <div>
      ...
      <div className="card overflow-hidden">
        ...
        : items.length === 0 ? (
          ...
          <tbody className="divide-y divide-gray-100">{items.map(item=><tr key={item.id} className="hover:bg-gray-50"><td className="px-4 py-3 text-xs font-mono text-gray-500">{item.doc_number??'—'}</td><td className="px-4 py-3 font-medium">{item.vendor_name}</td><td className="px-4 py-3 text-xs text-gray-500">{item.work_type}</td><td className="px-4 py-3 text-xs text-gray-600">{item.provision_date}</td><td className="px-4 py-3 text-xs text-gray-500">{(item.info_items??[]).length}개</td><td className="px-4 py-3 text-xs text-gray-500">{item.receiver_name||'—'}</td><td className="px-4 py-3"><span className={clsx('text-xs px-2 py-0.5 rounded-full font-medium',item.status==='completed'?'bg-green-50 text-green-700':'bg-amber-50 text-amber-700')}>{item.status==='completed'?'완료':'작성 중'}</span></td></tr>)}</tbody></table>
        )}
      </div>
    </div>
  )
