...
export default function SafetyInfoPage() {
  ...
  return (
    <div>
      ...
      <div className="card p-4 mb-4 bg-green-50/40 border-green-100 text-xs text-green-700 leading-relaxed rounded-xl">산안법 제65조: 도급인은 관계수급인 근로자가 도급인의 사업장에서 작업을 하는 경우, 다음 각 호의 정보를 해당 작업 시작 전에 관계수급인에게 문서로 제공해야 한다. ① 안전·보건에 관한 지식 또는 경험 ② 유해·위험 정보</div>
      <div className="card overflow-hidden">
        ...
        : items.length === 0 ? (
          ...
          <tbody className="divide-y divide-gray-100">{items.map(item=><tr key={item.id} className="hover:bg-gray-50"><td className="px-4 py-3 text-xs font-mono text-gray-500">{item.doc_number??'—'}</td><td className="px-4 py-3 font-medium">{item.vendor_name}</td><td className="px-4 py-3 text-xs text-gray-500">{item.work_type}</td><td className="px-4 py-3 text-xs text-gray-600">{item.provision_date}</td><td className="px-4 py-3 text-xs text-gray-500">{(item.info_items??[]).length}개</td><td className="px-4 py-3 text-xs text-gray-500">{item.receiver_name||'—'}</td><td className="px-4 py-3"><span className={clsx('text-xs px-2 py-0.5 rounded-full font-medium',item.status==='completed'?'bg-green-50 text-green-700':'bg-amber-50 text-amber-700')}>{item.status==='completed'?'완료':'작성 중'}</span></td><td className="px-4 py-3"><Link href={`/subcontract/safety-info/${item.id}`} className="text-xs text-blue-600 hover:underline">상세</Link></td></tr>)}</tbody></table>
        )}
      </div>
    </div>
  )
