...
export default function Page() {
  const role = SAFETY_ROLES['safety_health_officer']
  const [items,   setItems]   = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/safety-documents?role_id=safety_health_officer')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/safety-management/safety-health-officer/new"
          className="btn-primary text-sm gap-1.5" style={{background:'#16a34a'}}>
          <Plus className="w-4 h-4"/> 안전보건관리담당자 작성
        </Link>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>안전보건관리담당자 문서가 없습니다.</p>
            <Link href="/safety-management/safety-health-officer/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#16a34a'}}>
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/safety-management/safety-health-officer/'+item.id} className="text-xs text-blue-600 hover:underline">상세</Link>
                    </td>
                  </tr>
                )
              })}
