...
export default function Page() {
  const role = SAFETY_ROLES['health_manager']
  const [items,   setItems]   = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/safety-documents?role_id=health_manager')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/safety-management/health-manager/new"
          className="btn-primary text-sm gap-1.5" style={{background:'#0891b2'}}>
          <Plus className="w-4 h-4"/> 보건관리자 작성
        </Link>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>보건관리자 문서가 없습니다.</p>
            <Link href="/safety-management/health-manager/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#0891b2'}}>
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/safety-management/health-manager/'+item.id} className="text-xs text-blue-600 hover:underline">상세</Link>
                    </td>
                  </tr>
                )
              })}
