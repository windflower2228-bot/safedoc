...
export default function Page() {
  const role = SAFETY_ROLES['industrial_physician']
  const [items,   setItems]   = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/safety-documents?role_id=industrial_physician')
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>산업보건의 문서가 없습니다.</p>
            <Link href="/safety-management/industrial-physician/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#7c3aed'}}>
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/safety-management/industrial-physician/'+item.id} className="text-xs text-blue-600 hover:underline">상세</Link>
                    </td>
                  </tr>
                )
              })}
