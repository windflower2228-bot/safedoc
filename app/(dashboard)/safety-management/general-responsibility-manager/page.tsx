...
export default function Page() {
  const role = SAFETY_ROLES['general_responsibility_manager']
  const [items, setItems] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/safety-documents?role_id=general_responsibility_manager')
      .then((r) => r.json())
      .then((j) => {
        setItems(j.data ?? [])
        setLoading(false)
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              {role.legalBasis} | {role.docType}
            </p>
          </div>
        </div>
        <Link href="/safety-management/general-responsibility-manager/new" className="btn-primary text-sm gap-1.5" style={{ background: role.color }}>
          <Plus className="w-4 h-4" /> {role.label} 작성
        </Link>
      </div>

      <div className="card p-4 mb-4 text-xs leading-relaxed rounded-xl" style={{ background: role.bg, color: role.color }}>
        {role.description}
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <Link href="/safety-management/general-responsibility-manager/new" className="btn-primary mt-4 text-sm inline-flex" style={{ background: role.color }}>
              ...
              {items.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/safety-management/general-responsibility-manager/' + item.id} className="text-xs text-blue-600 hover:underline">
                        상세
                      </Link>
                    </td>
                  </tr>
