...
export default function Page() {
  const role = SAFETY_ROLES['honorary_inspector']
  const [items,   setItems]   = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/safety-documents?role_id=honorary_inspector')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/safety-management/honorary-inspector/new"
          className="btn-primary text-sm gap-1.5" style={{background:'#ea580c'}}>
          <Plus className="w-4 h-4"/> 명예산업안전감독관 작성
        </Link>
      </div>

      <div className="card p-4 mb-4 text-xs leading-relaxed rounded-xl"
        style={{background:'#ea580c10',color:'#ea580c'}}>
        {role?.description}
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>명예산업안전감독관 문서가 없습니다.</p>
            <Link href="/safety-management/honorary-inspector/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#ea580c'}}>
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/safety-management/honorary-inspector/'+item.id} className="text-xs text-blue-600 hover:underline">상세</Link>
                    </td>
                  </tr>
                )
              })}
