...
export default function HealthCheckPage() {
  ...
  useEffect(() => {
    fetch('/api/health/health-check?type=placement')
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <Stethoscope className="w-5 h-5" style={{color:'#7c3aed'}}/>배치전 건강진단
            </h1>
            <p className="text-sm text-gray-400 mt-0.5">산안법 제130조 | 유해업무 배치 전 필수</p>
          </div>
        </div>
        <Link href="/health/health-check-placement/new" className="btn-primary text-sm gap-1.5" style={{background:'#7c3aed'}}>
          <Plus className="w-4 h-4"/> 진단결과 등록
        </Link>
      </div>
      <div className="card p-4 mb-4" style={{background:'#f5f3ff',borderColor:'#7c3aed20'}}>
        <p className="text-xs leading-relaxed" style={{color:'#7c3aed'}}>근로자를 유해업무에 배치하기 전에 실시 | 업무 적합성 평가 및 직업병 예방 | 배치 전 반드시 실시 필수</p>
      </div>
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-14 text-center text-sm text-gray-400">
            ...
            <p>건강진단 결과가 없습니다.</p>
            <Link href="/health/health-check-placement/new" className="btn-primary mt-4 text-sm inline-flex" style={{background:'#7c3aed'}}>
              <Plus className="w-4 h-4"/> 진단결과 등록
            </Link>
          </div>
        ) : (
