...
export default function NewPage() {
  ...
  async function onSubmit(data: any) {
    setSaving(true)
    const res = await fetch('/api/health/special', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ...data, type:'special' }) })
    const json = await res.json()
    setSaving(false)
    if (!res.ok) { toast.error(json.error||'저장 실패'); return }
    toast.success('저장되었습니다.')
    router.push('/health/health-check-special')
  }
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/health/health-check-special" className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"><ArrowLeft className="w-4 h-4"/></Link>
          <div><h1 className="text-lg font-bold text-gray-900">특수건강진단 등록</h1><p className="text-xs text-gray-400">산안법 제131조 | 유해인자별 주기</p></div>
        </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <div className="card p-5 space-y-4">
          <div><label className="label-base">제목 *</label><input {...form.register('title',{required:true})} className="input-base" placeholder="특수건강진단 결과 등록"/></div>
